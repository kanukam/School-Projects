...
class ArrayQueue
{
	friend ostream & operator<<(ostream & os,const ArrayQueue & arrayQueue); //(i)
	 public:
		ArrayQueue();						//(1)
		ArrayQueue(size_t count, const DataType & value);	//(2)
		//ArrayQueue(const ArrayQueue & other);			//(3)			 //Not needed no dynamic memory involved
		~ArrayQueue();						//(4)
		//ArrayQueue & operator=(const ArrayQueue & rhs);		//(5)            //Not needed no dynamic memory involved
		DataType & front();					//(6a)
