...
class NodeList{
  friend std::ostream & operator<<(std::ostream & os, const NodeList & nodeList);
  ...
  public:
   ...
   ~NodeList();							   			  //(4)
    ...
    Node * find(const DataType & target,Node * & previous);	//(8) 
	
    Node * insertAfter(const DataType & target,const DataType & value);	//(9)     
    Node * insertBefore(const DataType & target,const DataType & value);//(10) 
